...
trait LogicalTrees {
  ...
  trait LogicalTrees { l: self.l.type =>
    ...
    object TermFunction {
      def unapply(tree: g.Function): Option[(List[g.Tree], g.Tree)] = {
        ...
        val lparams = params.map(_.with(TermParamRole))
        Some((lparams, body))
      }
    }
    ...
    object TypeIdent {
      def unapply(tree: g.Tree): Option[l.TypeName] = {
        val g.Ident(name) = tree
    ...
    object TypeParamDef {
      def unapply(tree: g.TypeDef): Option[(List[l.Modifier], l.TypeName, List[g.TypeDef], g.TypeBoundsTree, List[g.Tree], List[g.Tree])] = {
        ...
        val ltparams = tparams.map(_.with(TypeParamRole(Nil, Nil)))
        val ltbounds = {
          rhs match {
        ...
        val TypeParamRole(lvbounds, lcbounds) = tree.as(TypeParamRole)
    ...
    object PatExtract {
      def unapply(tree: g.Tree): Option[(g.Tree, List[g.Tree], List[g.Tree])] = {
        ...
        val largs = args.map(_.with(PatLoc))
    ...
    object ClassDef {
      def unapply(tree: g.ClassDef): Option[(List[l.Modifier], l.TypeName, List[g.TypeDef], g.DefDef, g.Template)] = {
        ...
        val lprimaryctor = gprimaryctor.with(PrimaryCtorRole(tree))
        val ltparams = applyBounds(tparams, lprimaryctor.vparamss)
        val ltempl = templ.with(TemplateRole(tree))
    ...
    object PackageObjectDef {
      def unapply(tree: g.PackageDef): Option[(List[l.Modifier], l.TermName, g.Template)] = {
        ...
        val PackageObjectPackageRole(g.ModuleDef(_, _, templ)) = tree.as(PackageObjectPackageRole)
        val ltempl = templ.with(TemplateRole(tree))
        Some((Nil, ???, ltempl))
      }
    }
    ...
    private object CtorDef {
      ...
      def unapply(tree: g.DefDef): Option[(Boolean, List[l.Modifier], l.CtorName, List[List[g.ValDef]], g.Tree, List[g.Tree])] = {
        ...
        val (lisPrimaryCtor, lname) = {
          if (tree.is(PrimaryCtorDef)) {
            val PrimaryCtorDef(lowner) = tree.as(PrimaryCtorDef)
            (true, lowner)
          } else if (tree.is(SecondaryCtorDef)) {
            val SecondaryCtorDef(lowner) = tree.as(SecondaryCtorDef)
            (false, lowner)
          } else {
            return None
          }
    ...
    object Template {
      def unapply(tree: g.Template): Some[(List[g.Tree], List[g.Tree], g.ValDef, List[g.Tree])] = {
        ...
        val TemplateRole(lowner) = tree.as(TemplateRole)
        ...
        val lparents = parents.zipWithIndex.map{ case (parent, i) =>
          ...
          lparent.with(SupercallLoc(superCtor.toLogical))
        }
      ...
      def unapply(tree: g.Tree): Option[(g.Tree, l.CtorIdent, List[List[g.Tree]])] = {
        ...
        (applied.callee, applied.core, applied.argss) match {
          case (tpt, classRef: g.RefTree, argss) =>
            val SupercallLoc(ctorSym) = tree.as(SupercallLoc)
    ...
    object SelfDef {
      def unapply(tree: g.ValDef): Option[(l.TermParamName, g.Tree)] = {
        ...
        val ldenot = {
          val SelfRole(lowner) = tree.as(SelfRole)
          val lsym = if (lowner.nonEmpty) l.Self(lowner.sym) else l.Zero
          l.Denotation(lowner.sym.thisPrefix, lsym)
        }
    ...
    private def applyBounds(tparams: List[g.TypeDef], paramss: List[List[g.ValDef]]): List[g.TypeDef] = {
      ...
      tparams.map(tparam => {
        ...
        tparam.with(TypeParamRole(vbounds, cbounds))
      })
    }
    ...
    private def removeBounds(paramss: List[List[g.ValDef]]): List[List[g.ValDef]] = {
      ...
      lparamss.map(_.map(_.with(TermParamRole)))
    }
  ...
  sealed private trait Location {
    def test(tree: g.Tree): Boolean = {
      val actual = {
        ...
        else if (tree.isInstanceOf[g.Template] || tree.metadata.contains("isLogicalSelf")) TemplateLoc
        else TermLoc
      }
      this == actual
    }
    def mark(tree: g.Tree): g.Tree = this match {
      ...
      case TemplateLoc => tree.appendMetadata("isLogicalSelf" -> true)
    }
  }
  ...
  @role private object TypeMemberRole extends Role[g.TypeDef] {
    ...
    def mark(tree: g.TypeDef) = tree.with(StatLoc)
  }
  ...
  object TypeParamRole {
    ...
    def set(tree: g.TypeDef, c: TypeParamRole): g.TypeDef = {
      val tree1 = tree.with(ParamLoc)
  ...
  object SelfRole {
    ...
    def set(tree: g.ValDef, c: SelfRole): g.ValDef = {
      tree.with(SelfLoc).appendMetadata("logicalOwner" -> c.lowner)
    }
  }
  ...
  object PrimaryCtorRole {
    ...
    def set(tree: g.DefDef, c: PrimaryCtorRole): g.DefDef = {
  ...
  object SecondaryCtorRole {
    ...
    def set(tree: g.DefDef, c: SecondaryCtorRole): g.DefDef = {
  ...
  object PackageObjectPackageRole {
    def get(tree: g.PackageDef) = tree match {
      case g.PackageDef(pid, List(mdef: g.ModuleDef)) if pkgobj.name == nme.PACKAGE =>
  ...
  object TemplateRole {
    ...
    def set(tree: g.Template, c: TemplateRole): g.Template = {
